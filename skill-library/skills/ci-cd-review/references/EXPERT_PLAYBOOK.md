...
- Map trigger to runner, build, test, artifact, and deploy.
- Review job/token permissions, forks/PRs with secrets, runners, and short-lived credentials.
- Review third-party actions/plugins, lockfiles, deterministic installs, and fail-closed tests.
- Separate dev/staging/prod credentials and promote trusted artifacts.
- Add provenance/attestation where justified and protect production deployment with gates.
- Verify rollback/roll-forward and post-deploy smoke tests.
...
- Coordinate with `container-production` when the problem materially crosses that boundary; do not duplicate its full scope.
