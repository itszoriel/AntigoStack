# Expert Playbook — `legacy-dotnet-modernization`
...
- Inventory target frameworks, project types, dependencies, COM/native components, databases, services, installers, and deployment environments.
- Identify business-critical workflows and establish regression tests before major migration.
- Separate language cleanup, package upgrades, architecture changes, and runtime migration into distinct steps.
- Check API/framework compatibility using current Microsoft tooling/documentation.
- Identify Windows-only and framework-only technologies that block direct migration.
- Review 32/64-bit, registry, filesystem, COM, Office, ODBC/OLE DB, P/Invoke, and vendor SDK dependencies.
- Prefer incremental multi-targeting, compatibility shims, or strangler-style boundaries when appropriate.
- Preserve data formats, config behavior, public interfaces, automation contracts, and installer expectations.
- Upgrade dependencies in controlled batches and test after each stage.
- Create rollback/side-by-side deployment plans for production migrations.
- Measure performance and memory behavior after runtime/framework changes.
- Document unsupported dependencies or areas requiring replacement rather than pretending migration is automatic.
...
- Coordinate with `aspnet-core-production` when the problem materially crosses that boundary; do not duplicate its full scope.
