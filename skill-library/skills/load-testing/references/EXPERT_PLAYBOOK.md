...
- Confirm authorization and a safe environment.
- Define workload, scenario mix, data, SLIs/SLOs, ramps, and thresholds.
- Establish a low-load baseline.
- Monitor app, DB, queue, cache, network, CPU, memory, and saturation.
- Progress from normal load to stress/spike/soak only when justified.
- Identify first saturated resource and retest after optimization.
