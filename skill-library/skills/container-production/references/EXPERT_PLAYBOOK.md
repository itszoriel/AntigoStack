# Expert Playbook — `container-production`
...
- Use trusted maintained bases and minimize runtime packages.
- Use multi-stage builds when useful and a restrictive .dockerignore.
- Keep secrets out of layers/build args and runtime config external.
- Run non-root where practical; keep mutable state outside image layers.
- Handle signals/shutdown and control versions/rebuilds.
- Build/test/scan images in CI and inventory the artifact.
...
- Coordinate with `kubernetes-production` when the problem materially crosses that boundary; do not duplicate its full scope.
