# Expert Playbook — `event-driven-architecture`
...
- Identify why asynchronous communication is justified.
- Distinguish commands, events, and queries; define ownership/source of truth.
- Define schemas/versioning and explicit ordering requirements.
- Assume duplicate delivery unless proven otherwise; design idempotent consumers.
- Define retries, DLQs, replay/backfill, and eventual-consistency UX.
- Add correlation/causation identifiers and AsyncAPI contracts where useful.
...
- Coordinate with `realtime-systems` when the problem materially crosses that boundary; do not duplicate its full scope.
