...
- Separate authentication from authorization and identify actors/trust boundaries.
- Prefer established protocols/providers over custom cryptography.
- Review redirect URIs, state, nonce, PKCE, issuer, audience, and token validation as applicable.
- Review session/token lifecycle, cookies/storage, logout, revocation, and CSRF implications.
- Review registration, recovery, MFA/passkeys, account linking, and service scopes.
- Enforce authorization server-side and test privilege/tenant boundaries.
