# Expert Playbook — `monorepo-build-tooling`
...
- Map packages/apps and dependency direction.
- Identify shared code and ownership boundaries.
- Prevent circular or forbidden dependencies.
- Centralize only configuration that is truly shared.
- Use deterministic workspace dependency resolution.
- Configure task dependency graph and caching safely.
- Run affected-only work where correctness permits.
- Review generated artifacts and cache invalidation.
- Define versioning/release strategy for internal/public packages.
- Keep CI local commands consistent.
