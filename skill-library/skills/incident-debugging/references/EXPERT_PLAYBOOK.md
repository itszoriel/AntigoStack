# Expert Playbook — `incident-debugging`

## Professional stance

Handle active production incidents: assess impact, contain safely, preserve evidence, test hypotheses, recover, verify, and document follow-up.
...
- Protect people/data/service and establish incident ownership/severity.
- Capture timeline, impact, known changes, current state, and evidence.
- Contain impact with reversible mitigations when possible.
- Use telemetry/deploy history to test hypotheses; avoid destructive speculative commands.
- Recover safely, verify user-facing behavior/data integrity, and monitor recurrence.
- Document root cause only when evidence supports it; add tests/runbook/corrective actions and a blameless timeline.
