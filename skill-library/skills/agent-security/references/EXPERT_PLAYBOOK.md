# Expert Playbook — `agent-security`
...
- Enumerate goals, tools, credentials, data sources, memory, and side effects.
- Treat prompts, webpages, documents, email, tool output, and retrieved content as untrusted.
- Separate instructions from data and minimize tool/credential permissions.
- Validate tool arguments/results outside model prose and add approval for consequential actions.
- Bound recursion, retries, spend, actions, time, and persistent memory writes.
- Test indirect prompt injection, exfiltration, confused-deputy, and cross-tenant scenarios with traces/evals.
...
- Coordinate with `mcp-security` when the problem materially crosses that boundary; do not duplicate its full scope.
