...
- Start from observable acceptance criteria and choose the lowest useful layer.
- Keep tests isolated/repeatable and seed/clean data predictably.
- Use stable user-facing locators and deterministic waiting.
- Test errors, permissions, and failure paths.
- Capture traces/screenshots/logs on failure.
- Investigate flakes before quarantine and run critical suites in CI.
...
- Coordinate with `bug-investigation` when the problem materially crosses that boundary; do not duplicate its full scope.
