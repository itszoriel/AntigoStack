...
- Inventory each server, tool, resource, prompt, owner, and update channel.
- Identify credentials/data accessible to server/client.
- Review tool descriptions and parameter schemas as security interfaces.
- Restrict filesystem/network/account/tenant scope and separate read from write tools.
- Require approval for consequential writes when appropriate.
- Treat returned content as untrusted and test prompt injection, revocation, and audit logging.
