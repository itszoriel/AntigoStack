# Expert Playbook — `kubernetes-production`
...
- Identify workload, namespace, tenant, and cluster assumptions.
- Review security context, Pod Security, service accounts/workload identity, secrets.
- Define resource requests/limits and startup/readiness/liveness probes.
- Review rollout, disruption, termination, network exposure/policies.
- Review volumes/backups, autoscaling, failure domains, logs/metrics/events.
- Validate rendered manifests and only apply to production with explicit authorization.
...
- Coordinate with `terraform-iac` when the problem materially crosses that boundary; do not duplicate its full scope.
