# Expert Playbook — `cloud-architecture`
...
- Define business criticality, workload profile, recovery needs, and constraints.
- Map components/trust boundaries and review identity/least privilege.
- Review networking, DNS/TLS, exposure, data durability, backup/recovery.
- Review failure domains, scaling/capacity, quotas, and managed-service limits.
- Review observability, deployment/change safety, performance, cost, and lock-in tradeoffs.
- Use current provider docs for concrete implementation.
...
- Coordinate with `finops-review` when the problem materially crosses that boundary; do not duplicate its full scope.
