# Expert Playbook — `backend-production`
...
- Identify service responsibilities/trust boundaries and validate external input.
- Enforce authorization server-side and keep secrets/config outside source/client output.
- Define transactions/consistency, timeouts, retry safety, and bounded concurrency/queues.
- Handle shutdown/termination and health/readiness behavior.
- Instrument important operations without leaking sensitive data and return safe errors.
- Test dependency outages, timeouts, partial failure, and deployment/resource assumptions.
