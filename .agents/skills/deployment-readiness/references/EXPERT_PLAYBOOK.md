# Expert Playbook — `deployment-readiness`

## Professional stance

Gate production deployment: config, secrets, infra, migrations, capacity, monitoring, backups, rollback, smoke tests, and ownership.
...
- Identify deployment target/architecture and verify production config/secrets.
- Verify DNS/TLS/network routes/origins, dependencies, and artifact provenance.
- Review migration ordering/compatibility, resources, capacity, autoscaling, and quotas.
- Verify health/readiness, logs/metrics/traces/alerts, backups/restore.
- Define deployment sequence, rollback/recovery, smoke tests, post-deploy checks, owner/on-call, and cost impact.
- Execute deployment only when explicitly requested/authorized.
