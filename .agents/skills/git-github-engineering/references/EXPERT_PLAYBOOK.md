# Expert Playbook — `git-github-engineering`

## Professional stance

Use Git/GitHub safely for branches, commits, PRs, reviews, tags, releases, CODEOWNERS, rulesets, and repository governance.
...
- Inspect current Git status, branches, remotes, hooks, and repository policy.
- Preserve uncommitted user work.
- Use focused commits and descriptive messages.
- Choose merge/rebase/squash strategy based on team policy.
- Review branch protection and required checks.
- Use CODEOWNERS/review rules where ownership matters.
- Tag/version releases consistently.
- Keep secrets and generated artifacts out of Git.
- Document contribution and release flow.
...
- Coordinate with `package-release-management` when the problem materially crosses that boundary; do not duplicate its full scope.
