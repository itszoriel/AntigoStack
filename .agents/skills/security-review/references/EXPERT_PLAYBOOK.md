# Expert Playbook — `security-review`

## Professional stance

Defensive security audit for auth, authorization, APIs, data, injection, business logic, secrets, dependencies, logging, abuse, and deployment.
...
- Establish assets, actors, trust boundaries, and scope.
- Review authentication/session lifecycle and authorization at every protected resource/action.
- Review validation, encoding, injection, file/URL/parser/SSRF-like surfaces, and business logic/races.
- Review sensitive data, cryptography, API object/function/property authorization, secrets, CORS/CSRF, headers, and environment separation.
- Review dependencies/supply chain, audit logging, abuse/rate limits, and recovery.
- Corroborate findings with authorized executable checks and re-test fixes.
...
- Coordinate with `auth-identity` when the problem materially crosses that boundary; do not duplicate its full scope.
- Coordinate with `software-supply-chain` when the problem materially crosses that boundary; do not duplicate its full scope.
