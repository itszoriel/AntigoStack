...
$Agents=Join-Path $Latest.FullName ".codex\AGENTS.md"
$Skills=Join-Path $Latest.FullName ".agents\skills"

if(Test-Path $Agents){
    if($PSCmdlet.ShouldProcess("$HOME\.codex\AGENTS.md","Restore prior AGENTS.md")){
        New-Item -ItemType Directory -Force "$HOME\.codex"|Out-Null
        Copy-Item $Agents "$HOME\.codex\AGENTS.md" -Force
    }
}
if(Test-Path $Skills){
    if($PSCmdlet.ShouldProcess("$HOME\.agents\skills","Restore prior Skills")){
